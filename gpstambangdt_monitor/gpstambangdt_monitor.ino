...
<head>
  ...
  <style>
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    /* Download buttons */
    .dl-card{background:linear-gradient(135deg,#1a1008,#2a1810);border-radius:12px;padding:16px;margin-bottom:12px;border:1px solid #3a2518}
    .dl-card h2{color:#ff9500;font-size:0.9em;margin-bottom:12px;padding-bottom:8px;border-bottom:1px solid #3a2518;letter-spacing:0.5px}
    ...
    .dl-jsonl{background:linear-gradient(135deg,#ff9500,#ff7700);color:#fff;box-shadow:0 4px 15px #ff950033}
    .dl-jsonl:hover{box-shadow:0 6px 20px #ff950055}
...
void handleDownloadJsonl() {
  ...
  webServer.sendHeader("Content-Disposition", "attachment; filename=\"DT01_gps_log.jsonl\"");
...
void handleDownloadCsv() {
  ...
  // Gunakan chunked transfer encoding untuk hemat memori
  webServer.sendHeader("Content-Disposition", "attachment; filename=\"DT01_gps_log.csv\"");
  webServer.sendHeader("Cache-Control", "no-cache");
  ...
  client.println("timestamp,latitude,longitude,speed,satellites,ignition,pto,imei,protocol,model,msg_id,source,record_type");
  ...
  while (f.available()) {
    ...
    csvLine += "\"" + String(doc["record_type"] | "") + "\"";
