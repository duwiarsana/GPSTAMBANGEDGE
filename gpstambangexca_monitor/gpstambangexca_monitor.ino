...
<head>
  ...
  <style>
    :root{--bg:#080a12;--card:#0f1220;--card2:#0a0d18;--border:#152035;--cyan:#06b6d4;--cyan2:#0891b2;--green:#22c55e;--red:#ef4444;--amber:#f59e0b;--txt:#c8cdd8;--dim:#4a5068;--dimmer:#2d3248}
    *{margin:0;padding:0;box-sizing:border-box}
    ...
    .bar .f{height:100%;background:linear-gradient(90deg,var(--cyan),var(--green));animation:s 3s linear infinite;will-change:width}
    @keyframes s{0%{width:100%}100%{width:0%}}

    .hdr{text-align:center;padding:20px 12px 16px;background:linear-gradient(160deg,#081520 0%,#0f1220 50%,#0a0d18 100%);border-radius:16px;margin-bottom:12px;border:1px solid #14303e;position:relative;overflow:hidden}
    .hdr::before{content:'';position:absolute;top:-50%;left:-50%;width:200%;height:200%;background:radial-gradient(circle at 70% 40%,rgba(6,182,212,.06) 0%,transparent 50%);pointer-events:none}
    .hdr h1{color:var(--cyan);font-size:1.25em;letter-spacing:1.5px;font-weight:800;position:relative}
    ...
    .c::after{content:'';position:absolute;top:0;left:0;right:0;height:1px;background:linear-gradient(90deg,transparent,rgba(6,182,212,.15),transparent)}
    .c h2{color:var(--cyan);font-size:.82em;margin-bottom:10px;padding-bottom:7px;border-bottom:1px solid var(--border);letter-spacing:.5px;font-weight:700}
    ...
    .s .v{font-size:1.1em;font-weight:700;color:var(--cyan);margin-top:3px}
    ...
    .fl{display:flex;justify-content:space-between;align-items:center;padding:7px 2px;border-bottom:1px solid rgba(21,32,53,.7)}
    ...
    .fl .vv.hi{color:var(--cyan);font-size:.95em;font-weight:800}
    ...
    th{background:linear-gradient(180deg,#081520,#0f1220);padding:8px 5px;text-align:left;color:var(--cyan);position:sticky;top:0;font-weight:700;letter-spacing:.5px}
...
<div class="hdr">
  <h1>🏗️ EXCA01 MONITOR</h1>
  <div class="sub"><span class="dot"></span>Excavator GPS Dashboard — Live</div>
</div>
...
<div class="ft">EXCA01 GPS Logger &copy; 2026 — Duwi Arsana</div>
...
function upd(){
  ...
  fetch('/api/logs?n=30').then(function(r){return r.json()}).then(function(d){
    ...
    for(var i=0;i<d.length;i++){var r=d[i],io=pi(r['in']);
      ...
      h+='<tr><td style="color:#3a3f52">'+(i+1)+'</td><td>'+(r.ts||'—')+'</td><td style="color:var(--cyan)">'+(r.lat!=null?r.lat.toFixed(6):'—')+'</td><td style="color:var(--cyan)">'+(r.lon!=null?r.lon.toFixed(6):'—')+'</td><td>'+(r.spd!=null?r.spd:'—')+'</td><td>'+pt+'</td><td>'+ac+'</td></tr>'}
...
void setup() {
  ...
  logMsg("=== " + String(EXCA_ID) + " MONITOR STARTING ===");
  ...
  logMsg("✅ " + String(EXCA_ID) + " MONITOR READY");
