    ...
    if (brace == 0) {
      ...
      if (processJSON(buf, clean)) {
        ...
        if (SD.exists(LOG_FILE)) {
          File tempFile = SD.open(LOG_FILE, FILE_READ);
          if (tempFile) {
        ...
        if (WiFi.status() == WL_CONNECTED && mqtt.connected()) {
          if (mqtt.publish(MQTT_DATA_TOPIC, clean.c_str())) {
...
void setup() {
  ...
  // Patched for arduino-esp32 2.0.x (IDF 4.4) API - same behavior as production
