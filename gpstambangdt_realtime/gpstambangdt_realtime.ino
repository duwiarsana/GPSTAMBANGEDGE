...
bool initStorage() {
  ...
  if (!ensureUintFile(DT_OFFSET_FILE, 0))
    return false;
  if (!ensureUintFile(RELAY_OFFSET_FILE, 0))
    return false;
  if (!ensureUintFile(DT_SEQ_FILE, 0))
    return false;
  ...
  while (Serial2.available()) {
    ...
    if (gpsBrace == 0) {
      ...
      if (processDTJson(gpsBuf, clean)) {
        uint32_t oldSize = 0;
        if (SD.exists(DT_LOG_FILE)) {
          File tempFile = SD.open(DT_LOG_FILE, FILE_READ);
          if (tempFile) {
            oldSize = tempFile.size();
            tempFile.close();
          }
        }
        uint32_t offset = readUint(DT_OFFSET_FILE, 0);
          ...
          if (WiFi.status() == WL_CONNECTED && client.connected()) {
            if (client.publish("kutai/fleet/data", clean.c_str())) {
              ...
              if (offset >= oldSize) {
                File tempFile = SD.open(DT_LOG_FILE, FILE_READ);
                if (tempFile) {
                  uint32_t newSize = tempFile.size();
                  tempFile.close();
                  writeUint(DT_OFFSET_FILE, newSize);
                  logMsg("⚡ Queue offset advanced (caught up)");
                }
              }
            } else {
              logMsg("⚠️ Real-time priority publish failed");
            }
          }
        }
      }
...
bool transferFromExca() {
  ...
  while (bytesReceived < totalToReceive &&
    ...
    if (client.available()) {
      int toRead =
          min((uint32_t)sizeof(buffer), totalToReceive - bytesReceived);
  ...
  if (!success || bytesReceived != totalToReceive) {
    logMsg("❌ Transfer incomplete. Got " + String(bytesReceived) + "/" +
           String(totalToReceive));
    SD.remove(tempPath);
    client.stop();
    return false;
  }
...
bool connectKnownInternet() {
  ...
  logMsg("🌐 Connecting: " + String(wifiList[bestIdx].ssid) +
...
bool publishQueueFile(const char *logPath, const char *offsetPath) {
  ...
  f.close();
  logMsg("✅ Published " + String(logPath) + ": " + String(sentCount) +
         " lines");
  return true;
}
...
// ================= HEARTBEAT =================
void printHeartbeat() {
  logMsg("💓 " + String(DT_ID) + " | GPS:" + String(statGpsLogged) +
         " | SKIP:" + String(statSkipped) + " | EXCA:" +
         String(statExcaRelayed) + " | MQTT:" + String(statMqttSent) +
...
void setup() {
  ...
  logMsg("✅ " + String(DT_ID) +
         " READY | IGN cooldown=" + String(IGN_COOLDOWN_MS / 1000) + "s");
}
