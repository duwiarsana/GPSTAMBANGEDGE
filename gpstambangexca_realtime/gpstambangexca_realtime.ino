  ...
  while(Serial2.available()){
    ...
    if(brace == 0){
      ...
      if(processJSON(buf, clean)){
        appendLog(clean);
        // Note: Standard EXCA runs strictly as a local WiFi Access Point server.
        // It has no direct internet route/MQTT client. Thus, real-time priority uploading
        // to MQTT is handled on the DT side (once connected to EXCA AP) or via the Hybrid EXCA variant.
      }
